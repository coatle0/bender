...
class TestDuplicateEventDedup:
    ...
    Without dedup, both handlers call pool.send() for the same
    thread_ts, racing ProcessPool._get_or_start()."""
    ...
    async def test_message_then_mention_for_same_physical_message_sends_once(
        self,
        setup_handler,
        session_manager: SessionManager,
        pool: AsyncMock,
        ...
        """Same (channel, ts) delivered as both event types -- only the
        first one processed should call pool.send()."""
        parent_ts = "3000000000.000001"
        reply_ts = "3000000000.500002"
        await session_manager.create_session(parent_ts)
        channel = "C123"
        pool.send.return_value = "ok"
        ...
        mention_event = dict(message_event)

        await setup_handler["message"](event=message_event, say=mock_say)
        await setup_handler["app_mention"](event=mention_event, say=mock_say, client=None)

        pool.send.assert_called_once_with(parent_ts, "확인 바람")

    async def test_mention_then_message_for_same_physical_message_sends_once(
        self,
        setup_handler,
        session_manager: SessionManager,
        pool: AsyncMock,
        ...
        """Same pair, opposite arrival order -- whichever runs first claims it."""
        parent_ts = "3000000000.000003"
        reply_ts = "3000000000.500004"
        await session_manager.create_session(parent_ts)
        channel = "C123"
        pool.send.return_value = "ok"
        ...
        await setup_handler["app_mention"](event=dict(event), say=mock_say, client=None)
        await setup_handler["message"](event=dict(event), say=mock_say)
